    ...
    def post(self, request):
        ...
        try:
            ...
            if group_by:
                ...
                if not agg_dict:
                    return Response({'error': 'Tidak ada kombinasi kolom dan jenis agregasi yang valid. (Contoh: SUM hanya untuk angka)'}, status=status.HTTP_400_BAD_REQUEST)
                ...
                for col in cols:
                    for t in types:
                        ...
                        if f:
                            ...
                            stats[f"{col}_{t}"] = val
                if not stats:
                    return Response({'error': 'Tidak ada perhitungan yang berhasil. Pastikan Anda memilih kolom angka untuk operasi matematika.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        ...
        try:
            ...
            if calc_type == 'pct_diff':
                ...
                df[result_name] = df[col_a] / df[col_b]
            ...
            df[result_name] = df[result_name].replace([float('inf'), float('-inf')], None)
